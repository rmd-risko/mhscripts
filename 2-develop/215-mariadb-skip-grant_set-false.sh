...
crudini --set /etc/mysql/mariadb.conf.d/50-server.cnf mysqld bind-address 127.0.0.1
vCrudINI_return=$?
if [ $vCrudINI_return -eq 0 ]; then
  echo 'Set bind-address to 127.0.0.1 in: /etc/mysql/mariadb.conf.d/50-server.cnf'
else
  echo 'Error, not set config in: /etc/mysql/mariadb.conf.d/50-server.cnf'
  exit $vCrudINI_return
fi
